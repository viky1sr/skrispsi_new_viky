...
@section('content')
    <div class="card card-custom gutter-b example example-compact">
        <!--begin::Form-->
        @role('member')
        <form id="buluTankisForm" method="post" action="{{empty($id) ? route('bulu-tangkis.store') : route('bulu-tangkis.update',$id)}}" enctype="multipart/form-data" >
            @endrole
            @role('admin')
            <form id="buluTankisForm" method="post" action="{{empty($id) ? route('bulu-tangkis.store-admin') : route('bulu-tangkis.update-admin',$id)}}" enctype="multipart/form-data" >
                @endrole
            ...
            <div class="card-footer">
                ...
                <a href="{{route('bulu-tangkis.index')}}">
        ...
        $(document).ready(() => {
            $('#buluTankisForm').on('submit', (e) => {
                ...
                $.ajax({
                    ...
                    data: new FormData($('#buluTankisForm')[0]),
                    dataType: 'json',
                    processData: false,
                    contentType: false,
                    beforeSend: function(){
