<nav id="sidebar">
    ...
    <ul class="list-unstyled menu-categories" id="accordionExample">
        ...
        <li class="menu">
            <a href="{{route('home')}}" aria-expanded="{{Route::is('home') ? "true" : "false"}}" class="dropdown-toggle">
                <div class="">
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-home"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"></path><polyline points="9 22 9 12 15 12 15 22"></polyline></svg>
        ...
        <li class="menu">
            <a href="{{route('resevation.index')}}"  aria-expanded="{{Route::is('resevation.index') ? "true" : "false"}}" class="dropdown-toggle">
                <div class="">
                    ...
                    @if(\App\HomeCare\Reservation\Reservation::where('status_id','=',1)->count() != 0 && \Auth::user()->hasrole('admin') ||
                        \App\HomeCare\Reservation\Reservation::where('status_id','=',1)->where('user_id','=',\Auth::user()->id)->count() != 0 && \Auth::user()->hasrole('member')
                    )
                        <span class="badge badge-primary">
                        @role('admin')
                            {{\App\HomeCare\Reservation\Reservation::where('status_id','=',1)->count()}}
                            @endrole

                            @role('member')
                            {{\App\HomeCare\Reservation\Reservation::where('status_id','=',1)->where('user_id','=',\Auth::user()->id)->count()}}
                            @endrole
                    </span>
                    @endif
                </div>
...
{{--        @endif--}}
        ...
        <li class="menu {{Route::is('payment.index') ? "active" : ""}}">
            <a href="{{route('payment.index')}}" aria-expanded="{{Route::is('payment.index') ? "true" : "false"}}"  class="dropdown-toggle">
                <div class="">
